...
# Method 1
# We convert the data into matrix because the model "glmnet" we are going to use needs data in matrix form
...
xtest <- data.matrix(test_data[,1:13])
ytest <- test_data$medv

# Method 2
x= model.matrix(medv~., train_data)[,-1]
# In the ablove expression where the formula is used "medv~., train_data", 
...
# From the plot note that the coefficients reduce when lambda increases. 
# However all 13 attributes remain, none of them are dropped. 
# Now we need to find the best value for lambda. 
# This may be done using the built-in cross validation of cv.glmnet. 
# Look up this function the value of k for CV
...
p1test <- predict(l_ridge_final, xtest) #remeber to use x and not train_data
rmse_l_ridge_final <- sqrt(mean((test_data$medv-p1test)^2))
rmse_l_ridge_final
...
# Prediction with training set
p2 <- predict(l_lasso_final, x) #remeber to use x and not train_data
rmse_l_lasso_final <- sqrt(mean((train_data$medv-p2)^2))
rmse_l_lasso_final

p2test <- predict(l_lasso_final, xtest) #remeber to use x and not train_data
rmse_l_lasso_final <- sqrt(mean((test_data$medv-p2test)^2))
rmse_l_lasso_final
